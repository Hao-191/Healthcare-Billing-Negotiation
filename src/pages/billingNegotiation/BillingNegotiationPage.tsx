...
// Define an interface for the issues
interface Issue {
    description: string;
    charged: number;
    expected: number;
...
const BillingNegotiationPage: React.FC = () => {
    ...
    const handleFileUpload = async (file: File) => {
      ...
      setTimeout(() => { // Simulate async operation, replace with actual file processing call
        setLoading(false);
        setIssues([{ description: "Duplicate Charge", charged: 350, expected: 175, confidence: 0.95 }]);
      }, 2000);
    };
    ...
    const handleCallInitiation = () => {
      ...
      setTimeout(() => { // Simulate async operation, replace with actual API call
        setCalling(false);
      }, 2000);
    };
